...
\subsection{Theory and Introduction}
Pattern recognition in digital images can be approached through morphological or statistical methods \cite{gonzalez2018}. This experiment compares two distinct techniques:
\begin{itemize}
    \item \textbf{Hit-or-Miss Transform (HMT):} A strict morphological tool used for exact shape detection in binary images. It utilizes a composite structuring element: $B_1$ to detect the foreground shape (the "hit"), and $B_2$ to detect the local background (the "miss"). A pixel is activated only if both conditions perfectly match.
          $$A \circledast B = (A \ominus B_1) \cap (A^c \ominus B_2)$$
    \item \textbf{Template Matching (Normalized Cross-Correlation):} A statistical method utilized for robust feature detection, especially in real-world scans. It slides a template across the image and calculates a correlation coefficient $[-1, 1]$. It is robust against noise and lighting variations.
\end{itemize}

\subsection{Methodology}
The two methods were implemented and evaluated on different types of datasets to demonstrate their respective strengths.

\subsubsection*{Part A: Morphological HMT}
A $10 \times 10$ synthetic binary matrix was defined containing various shapes. $SE_1$ was defined as a $2 \times 2$ matrix of 1s to detect a square, while $SE_2$ was defined as a $2 \times 2$ matrix of 0s. The SciPy \verb|binary_hit_or_miss| function was utilized to strictly isolate the target geometry.

\subsubsection*{Part B: Template Matching}
A real-world scanned text image was processed. The input was binarized using Otsu's thresholding. A Region of Interest (ROI) containing the letter "e" was defined as the template. The OpenCV \verb|matchTemplate| function was utilized, followed by Non-Maximum Suppression (NMS) via \verb|peak_local_max| to isolate unique detection peaks above a $0.8$ threshold.

\subsection{Python Implementations}
The implementations for both approaches are provided below.

\subsubsection*{HMT Implementation}
\begin{multicols}{2}
    \inputminted{python3}{./assets/lab9.py}
\end{multicols}

\subsubsection*{Template Matching Implementation}
\begin{multicols}{2}
    \inputminted{python3}{./assets/lab9_v2.py}
\end{multicols}

\subsection{Results}
The detection algorithms were executed, and the localized patterns were visualized.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{lab9.png}
    \caption{Hit-or-Miss Transform Results. Left: Synthetic $10 \times 10$ matrix containing $2 \times 2$ squares alongside larger rectangles and noise. Right: The single coordinate representing the exact location of the strict $2 \times 2$ match.}
    \label{fig:hmt_synthetic}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Template Matching Results. Left: The extracted binary template of the letter "e". Right: The detection results, showing 5 localized instances marked with red bounding boxes despite surrounding text noise.}
    \label{fig:tm_results}
\end{figure}

\subsection{Discussion}
The comparison highlighted the fundamental differences between morphological and statistical pattern recognition.

In Part A, the stringent matching criteria of the HMT were clearly demonstrated. The transform successfully isolated the exact $2 \times 2$ square but completely ignored larger blocks of 1s (such as the $2 \times 3$ rectangle). A positive result was only returned where the arrangement of 1s and surrounding 0s strictly matched the combined constraints of $SE_1$ and $SE_2$.

In Part B, the Normalized Cross-Correlation approach proved highly effective for feature detection in a noisy scan. All 5 instances of the lowercase "e" were perfectly isolated. Unlike the HMT, the statistical approach successfully ignored similar geometric shapes (such as the rounded bowls in the letters "d" and "a") because their cross-correlation coefficients fell below the 0.8 threshold, demonstrating flexibility that strict morphological operators lack.

\subsection{Conclusion}
The strengths of both pattern recognition techniques were validated. It was shown that the Hit-or-Miss Transform is an extremely precise tool for structural analysis in pure binary matrices where pixel-perfect accuracy is required. Conversely, Template Matching via Normalized Cross-Correlation was proven to be superior for Optical Character Recognition (OCR) tasks and real-world image processing, where robustness against noise and slight morphological variations is necessary.
